...
public class MsuGenerationWindowService(MsuPcmService msuPcmService, ProjectService projectService, StatusBarService statusBarService, TrackListService trackListService) : ControlService
{
    ...
    private async Task PerformAction(MsuGenerationRowViewModel rowDetails, ConcurrentBag<MsuGenerationRowViewModel>? toRetry = null)
    {
        ...
        {
            rowDetails.Message = projectService.CreateSmz3SplitRandomizerYaml(_model.MsuProject, true, false, out var error)
                ? "Generated"
        ...
        {
            rowDetails.Message = projectService.CreateSmz3SplitRandomizerYaml(_model.MsuProject, false, true, out var error)
                ? "Generated"
                : error ?? "Could not generate YAML file";
            _model.SongsCompleted++;
        }
    ...
    private void Compress(MsuGenerationRowViewModel compressRow)
    {
        try
        {
            ...
            foreach (var row in _model.Rows.Where(row => row.Type != MsuGenerationRowType.Compress).TakeWhile(row => !_cts.Token.IsCancellationRequested))
            {
